...
async function fetchTenderDetails(contract) {
    ...
    try {
        ...
        const projectAmount = ethers.utils.formatUnits(result[4], 18); 
        const validDate = new Date(result[8] * 1000); 


        document.getElementById('project-title').innerHTML = projectTitle;
        document.getElementById('project-amount').innerHTML = `Amount: RM ${projectAmount}`;
        document.getElementById('tender-id').innerHTML = tenderID;
        document.getElementById('validity-expiry').innerHTML = validDate.toLocaleDateString();

        // Timer Logic
        calculateDeadline(validDate);
...
// Function to dynamically update the link with the tenderID
async function updateLink() {
    const tenderID = new URLSearchParams(window.location.search).get('tenderID');

    if (tenderID) {
        // Get the back link element by its ID
        const backLink = document.getElementById('bidder-list');
        
        // Update the href with the tenderID
        backLink.href = `bidder_list.html?tenderID=${tenderID}`;
    }
}
